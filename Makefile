...
run:
	...
	PYTHONPATH=src uv run uvicorn shiftagent_api.api.app:app --host 0.0.0.0 --port 8081 --reload
...
run-https:
	...
	PYTHONPATH=src uv run uvicorn shiftagent_api.api.app:app --host 0.0.0.0 --port 8081 --reload --ssl-keyfile=./localhost-key.pem --ssl-certfile=./localhost.pem
...
debug:
	...
	PYTHONPATH=src uv run uvicorn shiftagent_api.api.app:app --host 0.0.0.0 --port 8081 --reload --log-level debug
...
run-streamlit:
	...
	cd src && PYTHONPATH=. uv run streamlit run shiftagent_ui/main.py --server.port 8501
