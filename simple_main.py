...
def analyze_weekly_hours(schedule: ShiftSchedule) -> Dict[str, Any]:
    ...
    for employee in schedule.employees:
        ...
        employee_analysis = {
            "name": employee.name,
            "employment_type": "フルタイム" if is_full_time_employee(employee) else "パートタイム",
    ...
    for week_key, summary in week_summary.items():
        analysis["by_week"][week_key] = {
            ...
            "average_hours_per_employee": round(
                summary["total_hours"] / len(summary["employees"]) if summary["employees"] else 0, 1
            )
        }
    
    # 統計情報の完成
    if total_employee_weeks > 0:
        analysis["statistics"]["average_weekly_hours"] = round(total_hours / total_employee_weeks, 1)
        analysis["statistics"]["compliance_rate"] = round(compliant_weeks / total_employee_weeks * 100, 1)
    
    if analysis["statistics"]["min_weekly_hours"] == float('inf'):
        analysis["statistics"]["min_weekly_hours"] = 0
    else:
        analysis["statistics"]["min_weekly_hours"] = round(analysis["statistics"]["min_weekly_hours"], 1)
    
    analysis["statistics"]["max_weekly_hours"] = round(analysis["statistics"]["max_weekly_hours"], 1)
