...
class TestAzureBlobJobStore:
    ...
    def mock_blob_service_client(self):
        ...
        with patch(
            "src.natural_shift_planner.api.azure_job_store.BlobServiceClient"
        ) as mock_client:
            mock_instance = Mock()
    ...
    def test_initialization_with_account_name(self, mock_blob_service_client):
        ...
        with patch(
            "src.natural_shift_planner.api.azure_job_store.DefaultAzureCredential"
        ):
...
def test_create_azure_job_store():
    ...
    with patch.dict(
        ...
        with (
            patch("src.natural_shift_planner.api.azure_job_store.BlobServiceClient"),
            patch(
                "src.natural_shift_planner.api.azure_job_store.DefaultAzureCredential"
            ),
        ):
