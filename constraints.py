from timefold.solver.score import (
    ConstraintFactory, Constraint, HardMediumSoftScore, 
    ConstraintCollectors, Joiners, constraint_provider
)
...
@constraint_provider
def shift_scheduling_constraints(constraint_factory: ConstraintFactory) -> list[Constraint]:
    """シフトスケジューリングの制約を定義"""
    return [
        # ハード制約（絶対に守る必要がある）
...
def required_skill_constraint(constraint_factory: ConstraintFactory) -> Constraint:
    ...
    return (constraint_factory
            ...
            .filter(lambda shift: (shift.employee is not None and 
                                  not shift.employee.has_all_skills(shift.required_skills)))
            .penalize(HardMediumSoftScore.ONE_HARD)
            .as_constraint("Required skill constraint"))


def no_overlapping_shifts_constraint(constraint_factory: ConstraintFactory) -> Constraint:
    ...
    return (constraint_factory
            ...
            .filter(lambda shift1, shift2: (shift1.employee is not None and 
                                          shift1.overlaps_with(shift2)))
            .penalize(HardMediumSoftScore.ONE_HARD)
...
def minimum_rest_time_constraint(constraint_factory: ConstraintFactory):
    ...
    return (constraint_factory
            ...
            .join(Shift,
                  Joiners.equal(lambda shift: shift.employee),
                  Joiners.less_than(lambda shift: shift.end_time, lambda shift: shift.start_time))
            .filter(lambda earlier_shift, later_shift:
                   (earlier_shift.employee is not None and
                    (later_shift.start_time - earlier_shift.end_time) < timedelta(hours=8)))
...
def minimize_unassigned_shifts_constraint(constraint_factory: ConstraintFactory):
    ...
    return (constraint_factory
            ...
            .penalize(HardMediumSoftScore.of_soft(1),
                     lambda shift: shift.priority * 10)
...
def fair_workload_distribution_constraint(constraint_factory: ConstraintFactory):
    ...
    return (constraint_factory
            ...
            .penalize(HardMediumSoftScore.ONE_SOFT,
                     lambda employee, total_minutes: abs(total_minutes - 480))  # 8時間 = 480分
...
def weekly_maximum_hours_constraint(constraint_factory: ConstraintFactory):
    ...
    return (constraint_factory
            ...
            .group_by(lambda shift: shift.employee,
                     ...
                     ConstraintCollectors.sum(lambda shift: shift.get_duration_minutes()))
            .filter(lambda employee, week, total_minutes: total_minutes > 45 * 60)  # 45時間超過で違反
            .penalize(HardMediumSoftScore.ONE_HARD,
...
def weekly_minimum_hours_constraint(constraint_factory: ConstraintFactory):
    ...
    return (constraint_factory
            .for_each(Shift)
            .filter(lambda shift: shift.employee is not None and is_full_time_employee(shift.employee))
            .group_by(lambda shift: shift.employee,
                     lambda shift: get_week_key(shift.start_time),
                     ConstraintCollectors.sum(lambda shift: shift.get_duration_minutes()))
            .filter(lambda employee, week, total_minutes: total_minutes < 32 * 60)  # 32時間未満
            .penalize(HardMediumSoftScore.ONE_MEDIUM,
...
def weekly_hours_target_constraint(constraint_factory: ConstraintFactory):
    ...
    return (constraint_factory
            ...
            .group_by(lambda shift: shift.employee,
                     ...
                     ConstraintCollectors.sum(lambda shift: shift.get_duration_minutes()))
            .penalize(HardMediumSoftScore.ONE_SOFT,
                     lambda employee, week, total_minutes: 
                     abs(total_minutes - get_target_hours(employee) * 60) // 60)
