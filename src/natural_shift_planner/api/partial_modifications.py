...
def check_shift_modification_constraints(
    ...
    for other_shift in schedule.shifts:
        if (other_shift.id != shift.id and 
    ...
    if weekly_hours > 40:
        warnings.append(f"Employee {new_employee.name} will have {weekly_hours:.1f} hours this week (overtime)")
    
    # Check minimum rest time if there are adjacent shifts
    min_rest_minutes = 8 * 60  # 8 hours
    for other_shift in schedule.shifts:
        if (other_shift.employee == new_employee and 
            other_shift.id != shift.id):
            # Check if shifts are adjacent
            if other_shift.end_time <= shift.start_time:
                rest_time = (shift.start_time - other_shift.end_time).total_seconds() / 60
                ...
                rest_time = (other_shift.start_time - shift.end_time).total_seconds() / 60
                if rest_time < min_rest_minutes:
                    violations.append(ConstraintViolation(
                        type="medium",
                        constraint_name="min_rest_time",
