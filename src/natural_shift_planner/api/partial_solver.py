...
def filter_shifts_by_scope(shifts: List[Shift], scope: OptimizationScope) -> Set[str]:
    """
    ...
    for shift in shifts:
        ...
        if scope.date_range:
            if not (
                scope.date_range.start_date
                <= shift.start_time
                <= scope.date_range.end_date
            ):
                continue
...
def create_partial_schedule(
    base_schedule: ShiftSchedule, scope: OptimizationScope, preserve_locked: bool
) -> ShiftSchedule:
    """
    ...
    new_employees = [
        Employee(e.id, e.name, e.skills.copy()) for e in base_schedule.employees
    ]
...
def solve_partial_schedule_async(
    job_id: str, base_schedule: ShiftSchedule, request: PartialOptimizationRequest
):
    """
    ...
    try:
        ...
        partial_schedule = create_partial_schedule(
            base_schedule, request.optimization_scope, request.preserve_locked
        )
        ...
        with job_lock:
            ...
            jobs[job_id]["partial_optimization_summary"] = {
                ...
                "shifts_modified": count_modified_shifts(
                    base_schedule, merged_schedule
                ),
            }
...
def merge_partial_solution(
    ...
    partial_assignments = {
        shift.id: shift.employee for shift in partial_solution.shifts
    }
    ...
    merged_schedule = ShiftSchedule(employees=base_schedule.employees, shifts=[])
...
def count_modified_shifts(original: ShiftSchedule, modified: ShiftSchedule) -> int:
    """
