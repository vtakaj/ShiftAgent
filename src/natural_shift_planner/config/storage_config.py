...
class StorageConfig:
    ...
    filesystem_dir: Optional[str] = None
...
class StorageConfigManager:
    ...
    def from_environment() -> StorageConfig:
        ...
        try:
            ...
            print(f"Warning: Unknown storage type '{storage_type_str}', defaulting to filesystem")
    ...
    def from_pulumi_outputs(pulumi_outputs: Dict[str, Any]) -> StorageConfig:
        ...
        return StorageConfig(
            ...
            azure_container_name=pulumi_outputs.get("storage_job_data_container", "job-data")
        )
    ...
    def validate_config(config: StorageConfig) -> bool:
        ...
        if config.storage_type == StorageType.FILESYSTEM:
            ...
            return (config.azure_connection_string is not None or 
                   config.azure_account_name is not None)
    ...
    def get_environment_variables(config: StorageConfig) -> Dict[str, str]:
        ...
        env_vars = {
            "JOB_STORAGE_TYPE": config.storage_type.value
        }
        ...
        if config.storage_type == StorageType.FILESYSTEM:
            ...
            if config.azure_connection_string:
                env_vars["AZURE_STORAGE_CONNECTION_STRING"] = config.azure_connection_string
